...
ENT.tblCRelationships = {
	[D_NU] = {"monster_gman", "npc_seagull", "npc_antlion_grub", "npc_barnacle", "monster_cockroach", "npc_pigeon", "npc_crow", "npc_enemyfinder", "npc_bullseye"},
...
function ENT:KeyValue(key, value)
	...
	if(key == "ondamaged") then self:StoreOutput(key,value); return end
...
function ENT:PlaySequence(name,loop,fcOnFinished)
	local sequenceID = self:LookupSequence(name)
	if(!sequenceID) then return end
	local anim = self:GetAnimInfo(sequenceID)
	self.m_tSequence = {
		ID = sequenceID,
		start = CurTime(),
		duration = anim.numframes /anim.fps,
		loop = loop,
		angles = self:GetAngles(),
		callOnFinished = fcOnFinished
	}
end

function ENT:AdvanceSequence()
	local seq = self.m_tSequence
	if(!seq) then return end
	local cycle = (CurTime() -seq.start) /seq.duration
	if(cycle > 1) then
		if(seq.loop) then cycle = cycle -math.floor(cycle)
		else
			self.m_tSequence = nil
			if(seq.callOnFinished) then seq.callOnFinished() end
			return
		end
	end
	//self:SetAngles(seq.angles)
	self:ResetSequence(seq.ID)
	self:SetCycle(cycle)
end

function ENT:SelectSchedule(iNPCState)
	if(self.m_tSequence) then return end
...
function ENT:OnCondition(iCondition)
	if self.bDead || self.m_tSequence then return end
...
function ENT:GetEnemies(ang)
	...
	for _, ent in ipairs(tbEnts) do
		if(ent != self && ent:Alive() && !ent:GetNoTarget() && ent:GetClass() != "npc_enemyfinder" && (self:GetAIType() != AI_TYPE_WATER || ent:WaterLevel() > 1) && !ent.bSelfDestruct) then
...
function ENT:OnTakeDamage(dmginfo)
	...
	self:TriggerOutput("ondamaged",entAttacker)
