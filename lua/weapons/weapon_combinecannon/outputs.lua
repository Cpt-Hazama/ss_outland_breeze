...
-- This is called from SWEP:KeyValue(key,value) to store the output from
-- the map, it could also be called from SWEP:AcceptInput I think, so if
...
-- If called from SWEP:KeyValue, then the first parameter is the key, and
-- the second is value.

function SWEP:StoreOutput(name, info)
...
function SWEP:TriggerOutput(name, activator)
